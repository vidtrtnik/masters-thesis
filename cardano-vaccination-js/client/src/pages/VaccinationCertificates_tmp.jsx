...
export default function VaccinationCertificates_tmp() {
  const today = new Date();
    ...
    const [patientAuthKey, setPatientAuthKey] = useState('');
  ...
  return ( !loading && !error && (
    ...
    <Col>
        <InputGroup className="mb-3">
        <InputGroup.Text id="basic-addon1">Patient DID Auth.Key</InputGroup.Text>
        <Form.Control type="text" maxLength={64} placeholder="Auth.Key (hex)" value={patientAuthKey} onChange={ (e) => setPatientAuthKey(e.target.value)} />
        <Form.Text className="text-muted">
        </Form.Text>
      </InputGroup>
      </Col>
